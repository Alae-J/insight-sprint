...
const MeetingsList = () => {
  ...
  useEffect(() => {
    const fetchMeetings = async () => {
      try {
        const projectData = await api.get<ProjectResponse>(`/api/projects/${id}`);
        const meetingData = await api.get<MeetingResponse[]>(`/api/projects/${id}/meetings`);
        setProject(projectData);
        setMeetings(meetingData);
      } catch (err) {
        console.error("Failed to fetch project or meetings", err);
      }
    };
    fetchMeetings();
  }, [id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      projectId: projectId,
      rawNotesMd: '',     // empty initially
      tags: [],
      attendeeIds: [],    // handled later
    };

    try {
      const createdMeeting = await api.post<MeetingResponse>('/api/meetings', payload);
    ...
    <div>
      <div className="mb-6">
        <div className="flex items-center text-sm text-text-secondary dark:text-gray-400 mb-4">
          <Link to="/projects" className="hover:text-primary-blue">Projects</Link>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="flex items-center text-text-secondary dark:text-gray-300">
                  ...
                  <span>Created by {meeting.creator.fullName}</span>
                </div>
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <Link to={`/meetings/${meeting.id}`} className="flex items-center text-primary-blue hover:underline">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md">
            <div className="p-6">
              <h2 className="text-xl font-semibold mb-4 text-text-primary dark:text-white">New Meeting</h2>
